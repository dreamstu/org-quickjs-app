...
exports.init = function(quick) {
    ...
    // Access files in the user's ".quick-init" folder.
    exports.userDir = function() {
        var dirpath = path.join.apply(path, arguments);
        var homepath = process.env[win32 ? 'USERPROFILE' : 'HOME'];
        return path.resolve(homepath, '.quick-init', dirpath);
    };
    ...
    exports.expand = function() {
        var args = quick.util.toArray(arguments);
        // If the first argument is an options object, remove and save it for later.
        var options = quick.util.kindOf(args[0]) === 'object' ? args.shift() : {};
        ...
        exports.searchDirs.forEach(function(dirpath) {
            ...
            // Set the cwd so the quick.file.expand* method can match relatively.
            opts.cwd = dirpath;
            ...
            quick.file.expand.apply(null, args).forEach(function(relpath) {
    ...
    exports.readDefaults = function() {
        ...
        if (!result) {
            ...
            filepaths = exports.searchDirs.map(function(dirpath) {
                ...
                return quick.file.isFile(filepath);
            });
            // Load defaults data.
            if (filepaths.length) {
                quick.verbose.subhead('Loading data from ' + filepath);
                // Since extras path order goes from most-specific to least-specific, only
                // add-in properties that don't already exist.
                filepaths.forEach(function(filepath) {
                    _.defaults(result, quick.file.readJSON(filepath));
                });
            }
        }
        return result;
